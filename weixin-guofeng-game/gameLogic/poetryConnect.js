...
PoetryConnectLogic.prototype.calcScore = function (elapsedSec) {
  var baseScore = 100
  var timePenalty = Math.floor((elapsedSec || 0) * 0.6)
...
PoetryConnectLogic.prototype.getScoreBreakdown = function (remainingSec, totalSec) {
  ...
  return {
    ...
    baseScore: 100,
    timePenalty: Math.floor(elapsedSec * 0.6),
    errorPenalty: this.wrongCount * 10,
...
PoetryConnectLogic.prototype.draw = function (ctx, offsetX, offsetY) {
  ...
  var connColors = [THEME.zhuSha, THEME.qingLan, '#2E7D32', '#6A1B9A', '#F57C00', '#00838F', '#AD1457', '#4E342E']
  for (var ci = 0; ci < this.connections.length; ci++) {
    ...
    if (from && to) {
      ctx.save()
      ctx.beginPath()
      var fx = from.x + from.width, fy = from.y + from.height / 2
      var tx = to.x, ty = to.y + to.height / 2
      ctx.moveTo(fx, fy)
  ...
  for (var idx = 0; idx < this.items.length; idx++) {
    ...
    if (isSel) ctx.fillStyle = 'rgba(200,37,6,0.1)'
    else if (isConn) ctx.fillStyle = '#F0F7F0'
    else ctx.fillStyle = '#FFFBF5'
    ctx.fill()
    ctx.restore()

    roundRect(ctx, it.x, it.y, it.width, it.height, 8)
    ctx.strokeStyle = isSel ? THEME.zhuSha : isConn ? '#4CAF50' : THEME.danJin
    ctx.lineWidth = isSel ? 2 : 1
    ctx.stroke()

    var fontSize = baseFontSize
    var maxWidth = it.width - 16
    var textLen = it.text.length
    if (textLen > 10) fontSize = Math.max(baseFontSize - 3, 10)
    else if (textLen > 7) fontSize = Math.max(baseFontSize - 2, 11)
    else if (textLen > 5) fontSize = Math.max(baseFontSize - 1, 12)
